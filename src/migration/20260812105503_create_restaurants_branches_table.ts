...
export async function up(knex: Knex): Promise<void> {
    await knex.raw(`
        ...
        CREATE INDEX idx_restaurant_branches_restaurant_id ON restaurants_branches(restaurant_id);
        CREATE INDEX idx_restaurant_branches_is_active ON restaurants_branches(is_active);
        CREATE INDEX idx_restaurant_branches_location ON restaurants_branches USING GIST(location);
    `)
}

export async function down(knex: Knex): Promise<void> {
    await knex.raw(`
        DROP TABLE IF EXISTS restaurants_branches;
